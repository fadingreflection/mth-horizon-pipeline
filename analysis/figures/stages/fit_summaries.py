...
def main():
    ...
    # Logistic regression requires both classes (0 and 1). Sparse custom
    # campaigns (e.g. a single success) must be skipped or sklearn raises
    # "data contains only one class".
    results = []
    skipped = []
    for agent_name, agent_df in runs_df.groupby("agent"):
        y = agent_df["score_binarized"].values
        n_classes = len(set(int(v) for v in y if pd.notna(v)))
        n_tasks = agent_df["task_id"].nunique()
        if n_classes < 2 or n_tasks < 2:
            skipped.append(
                f"{agent_name}: n_tasks={n_tasks}, classes={sorted(set(int(v) for v in y if pd.notna(v)))}"
            )
            continue
    ...
    if skipped:
        print("Skipped models (need >=2 tasks and both score classes 0/1 for IRT):")
        for msg in skipped:
            print(f"  - {msg}")

    if not results:
        raise RuntimeError(
            "No models could be fit for IRT — every agent lacked both "
            "success and failure scores (or had <2 tasks)."
        )
