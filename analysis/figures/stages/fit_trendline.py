...
def main():
    ...
    if len(sota) < 2:
        ...
        skipped = {
            "skipped": True,
            "reason": f"Only {len(sota)} SOTA models — need at least 2 for trendline",
            "n_sota": int(len(sota)),
            "doubling_time_days": None,
        }
        out.write_text(json.dumps(skipped, indent=2))
        print(f"Skipped trendline fit: {skipped['reason']}")
