...
def _load_expert_rates() -> tuple[dict[str, float], dict[str, float]]:
    """Load expert rates from experts.json if present.
    ...
    Public / custom checkouts may ship an empty placeholder ``{}`` or omit the
    file entirely — in that case return empty maps (expert-rate plots skip).
    """
    import json
    from pathlib import Path

    # Prefer analysis/experts.json (dvc wdir is analysis/); also try CWD.
    candidates = [
        Path(__file__).resolve().parent.parent / "experts.json",
        Path("experts.json"),
    ]
    experts_file = next((p for p in candidates if p.exists()), None)
