# Инструкция: кастомные `.eval` → time horizon (P50 / trendline)

Репозиторий анализа: `cyber-task-horizons-data`  
Источник логов Inspect: обычно `inspect_evals/logs/*.eval`

Цель: после прогона модели через Inspect получить **P50 time horizon** и (при ≥2 моделях с датами) **doubling time** на trendline.

Списки CyberGym-задач с human minutes — в [Приложении A](#приложение-a-cybergym-задачи-с-человекоминутами).

---

## 0. Что измеряет пайплайн (кратко)
...
## 1. После получения кастомных `.eval`
...
## 2. Как прогнать пайплайн (только time horizon)
...
## 4. Сводка изменений в коде и зачем они
...
## 5. Чеклист перед «серьёзным» горизонтом
...
## 6. Текущее состояние (пример успешного прогона)
